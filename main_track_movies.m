...
% Path to Mat file
path = '/Users/hannes/fret-ibra/ibra/GCamp3_cropped2'; % Input folder path (ADD PATH TO FILE HERE)
...
distributions = 0; d = 1;  % Show histogram of results in the end. d is counter
...
pixelsize = 0.2; % Pixel to um conversion

% Kymo, movie and measurements options
Cmin = 30; % Min pixel value in Ratio stack
Cmax = 135; % Max pixel value in Ratio stack
nkymo = 5; % Number of pixels line width average for kymograph (odd number) (0 means no kymo)
diamcutoff = 6; % In pixels if pixelsize is not given
...
if single_channel == 0
    ...
    maxInt = max(M(:));
    for count = 1:size(M,3)
        Ms = M(:,:,count);
        Ms = uint8(single(Ms).*255/single(maxInt));
        level1 = graythresh(Ms);
        MsB = imbinarize(Ms,level1);
        Ms = uint16(Ms).*uint16(MsB);
        M(:,:,count) = Ms;
    end
    BT1 = M; BT2 = M;
end
...
for count = smp:-1:stp
    try
        ...
        Q3geo(isnan(Q3geo)|isinf(Q3geo)) = 0;
        ...
        if (ROItype > 0)
            ...
            elseif (type == 2) BT1r = BT1(:,:,count); BT2r = BT2(:,:,count);
            ...
            if (max(O(:)) <= 255) FO = uint8(F); % can we use O here, or should we use uint8(M(:,:,count))? O is uint8(M(:,:,count)).*255
            else FO = uint16(F);
            end
            F = uint16(F);
...
try
    ...
    subplot(3,1,2)
    diamf_matrix=[[stp:smp]',diamf_avg(stp:smp)'];
    [diamf_rmout,Tfrm]=rmoutliers(diamf_matrix); % remove outliers
    diamf_rmout_smth = smoothdata(diamf_rmout(:,2), 'movmean', 5); % smooth diameter
    plot(stp:smp,diamf_avg(stp:smp),'b')
    hold on
    plot(find(~Tfrm), diamf_rmout_smth, 'r','LineWidth',2)
    hold off
    ...
    saveas(gcf, [path, '/TM_figures/', 'Diameter'], 'fig');
    saveas(gcf, [path, '/TM_figures/', 'Diameter'], 'svg');
catch
...
saveas(gcf, [path, '/TM_figures/', 'Kymograph'], 'fig');
saveas(gcf, [path, '/TM_figures/', 'Kymograph'], 'svg');
...
if (ROItype > 0)
    ...
    if single_channel == 0
        if (split)
            F1ratio = intensityB1_F1(stp:smp)./intensityB2_F1(stp:smp); %add subplot where intensityM_Fx is plotted
            subplot(1,5,2)
            hold on
            ...
            subplot(1,5,4);
            plot(stp:smp,intensityM_F1,'b')
            axis([stp-1 smp+1 0.8 max(intensityM_F1(:))*1.25]);
            title('Intensity F1 (split ROI 1)'); xlabel('Frame');
            
            subplot(1,5,5);
            plot(stp:smp,intensityM_F2,'b')
            axis([stp-1 smp+1 0.8 max(intensityM_F2(:))*1.25]);
            title('Intensity F2 (split ROI 1)'); xlabel('Frame');
        else
        ...
        if (split)
            F1ratio = intensityB1_F1(stp:smp);
            subplot(1,3,2)
            hold on
            plot(stp:smp,F1ratio,'b')
            axis([stp-1 smp+1 0.8 max(F1ratio(:))*1.25]);
            ...
            F2ratio = intensityB1_F2(stp:smp);
            subplot(1,3,3)
            hold on
            plot(stp:smp,F2ratio,'b')
            axis([stp-1 smp+1 0.8 max(F1ratio(:))*1.25]);
    ...
    saveas(gcf, [path, '/TM_figures/', 'Intensities'], 'fig');
    saveas(gcf, [path, '/TM_figures/', 'Intensities'], 'svg');
end
...
if (distributions == 1)    
    figure
    subplot(1,2,1)
    histogram(Mhist(Mhist(:,1)>0.1))
    hold on; histogram(Mhist(Mhist(:,2)>0.1))
    title('Histogram C')
    
    subplot(1,2,2)
    histogram(MhistF(MhistF(:,1)>0.1))
    hold on; histogram(MhistF(MhistF(:,2)>0.1))
    title('Histogram CF')
    
    figure
    subplot(1,2,1)
    histogram(B1hist(B1hist(:,1)>0.1))
    hold on; histogram(B1hist(B1hist(:,2)>0.1))
    title('Histogram B1')
    
    subplot(1,2,2)
    histogram(B1histF(B1histF(:,1)>0.1))
    hold on; histogram(B1histF(B1histF(:,2)>0.1))
    title('Histogram B1F')
    
    figure
    subplot(1,2,1)
    histogram(B2hist(B2hist(:,1)>0.1))
    hold on; histogram(B2hist(B2hist(:,2)>0.1))
    title('Histogram B2')
    
    subplot(1,2,2)
    histogram(B2histF(B2histF(:,1)>0.1))
    hold on; histogram(B2histF(B2histF(:,2)>0.1))
    title('Histogram B2F')
end
